...
// Client operation handlers for "pcc" overload control algorithm
type CpccOps struct{}

func cpccSendCUpdate(ops *CpccOps, cc *CpccConn) int {
	s := cc.Session
...
func cpccSendRaw(ops *CpccOps, cc *CpccConn, buf []byte, len uint64, id uint64) int {
	s := cc.Session
...
func cpccSendRequestVector(ops *CpccOps, cc *CpccConn) int {
	s := cc.Session
	now := MicroTime()
...
func cpccDrainQueue(ops *CpccOps, s *CpccSession) {
	now := MicroTime()
	...
	for s.Head != s.Tail {
		pos := s.Tail % CrpcQlen
		c := s.QReq[pos]
		if CpccMaxClientDelayUs == 0 || now-c.Ts <= CpccMaxClientDelayUs {
			break
		}
...
func cpccEnqueueOne(ops *CpccOps, s *CpccSession, buf []byte, len uint64, arg any) bool {
	now := MicroTime()
	...
	if !s.Init {
		for i := 0; i < s.Cmn.NConns; i++ {
			cc := s.Cmn.C[i].Ext.(*CpccConn)
			cpccSendCUpdate(ops, cc)
			cc.WaitingResp = true
		}
		s.Init = true
	}
...
func (ops *CpccOps) CrpcAddConnection(s *CrpcSession, raddr *net.TCPAddr) int {
	ss := s.Ext.(*CpccSession)
...
func (ops *CpccOps) CrpcSendOne(s *CrpcSession, buf []byte, len uint64, hash int, arg any) int {
	ss := s.Ext.(*CpccSession)
	...
	// Hot path, just send
	cc := ss.Cmn.C[ss.NextConnIdx].Ext.(*CpccConn)
	if cc.CreditUsed < cc.Credit && ss.Head == ss.Tail {
		cc.CreditUsed++
		ret := cpccSendRaw(ops, cc, buf, len, ss.ReqId)
		ss.ReqId++
	...
	if !cpccEnqueueOne(ops, ss, buf, len, arg) {
		cpccDrainQueue(ops, ss)
		ss.Lock.Unlock()
		return -1
	}
	cpccDrainQueue(ops, ss)
...
func (ops *CpccOps) CrpcRecvOne(c *CrpcConn, buf []byte, len uint64, arg any) int {
...
again:
	...
	case PccOpCall:
		...
		if shdr.Len > 0 {
			...
			if shdr.Flags&uint8(PccSFlagDrop) == 0 {
				cc.RespRx++
			}
		}
...
func cpccTimer(ops *CpccOps, s *CpccSession) {
	...
	for {
		...
		for s.Head != s.Tail {
			pos := s.Tail % CrpcQlen
			c := s.QReq[pos]
			if now-c.Ts <= CpccMaxClientDelayUs {
				break
			}
		...
		now = MicroTime()
		if now < c.Ts+CpccMaxClientDelayUs {
			sleepDuration := c.Ts + CpccMaxClientDelayUs - now
...
func (ops *CpccOps) CrpcClose(s *CrpcSession) {
	ss := s.Ext.(*CpccSession)
	...
	// Free the connections. Break the CrpcConn <-> CpccConn cycle (and
	// CpccConn -> CpccSession back-ref) before dropping references. Cycles
	...
	// Break the CrpcSession <-> CpccSession cycle. Safe to do here because
...
func (ops *CpccOps) CrpcCredit(s *CrpcSession) uint64 {
	ss := s.Ext.(*CpccSession)
	ret := uint64(0)

	for i := 0; i < ss.Cmn.NConns; i++ {
		c := ss.Cmn.C[i].Ext.(*CpccConn)
		ret += uint64(c.Credit)
	}
...
func (ops *CpccOps) CrpcStatClear(s *CrpcSession) {
	ss := s.Ext.(*CpccSession)
...
func (ops *CpccOps) CrpcStatECreditRx(s *CrpcSession) uint64 {
	ss := s.Ext.(*CpccSession)
	ret := uint64(0)
...
func (ops *CpccOps) CrpcStatCreditExpired(s *CrpcSession) uint64 {
	ss := s.Ext.(*CpccSession)
	ret := uint64(0)
...
func (ops *CpccOps) CrpcStatCUpdateTx(s *CrpcSession) uint64 {
	ss := s.Ext.(*CpccSession)
	ret := uint64(0)
...
func (ops *CpccOps) CrpcStatRespRx(s *CrpcSession) uint64 {
	ss := s.Ext.(*CpccSession)
	ret := uint64(0)
...
func (ops *CpccOps) CrpcStatReqTx(s *CrpcSession) uint64 {
	ss := s.Ext.(*CpccSession)
	ret := uint64(0)
...
func (ops *CpccOps) CrpcStatReqDropped(s *CrpcSession) uint64 {
	ss := s.Ext.(*CpccSession)
