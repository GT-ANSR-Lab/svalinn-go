...
type g struct {
	...
	// Timestamp (in ns) when G was marked ready and added to the runq/netq
	enqueTime int64
}
...
type p struct {
	...
	// Most recent queueing delay (ns) observed when a goroutine was
	// dequeued from this P's runq. Updated in execute() on every
	// goroutine scheduling event. Written only by the owning M.
