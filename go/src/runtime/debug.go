...
// Return the G (Go routine) id of the current G
func GetGoId() int {
	gp := getg()
	return int(gp.goid)
}

// Return the P (Proc) id on which the current G is running
//
// Note: The same call might return different P(s) for the same G, as
//       Go preempts G(s) periodically.
func GetProcId() int {
	gp := getg()
	mp := gp.m
