...
func casgstatus(gp *g, oldval, newval uint32) {
	...
	if newval == _Grunnable {
		// The go routine is going to be enqueued in a runq soon.
		// Record the enqueue time here. This covers all the cases of
		// the go routine being enqueued either in the local runq or the global
		// runq.
		gp.enqueTime = nanotime()
	}
	if !gp.tracking {
		return
	}
...
func execute(gp *g, inheritTime bool) {
	...
	// Update per-P queueing delay estimate.
	// enqueTime was set when gp transitioned to _Grunnable in casgstatus.
	if gp.enqueTime != 0 {
		delay := nanotime() - gp.enqueTime
		mp.p.ptr().maxQueueDelay.Store(delay)
		gp.enqueTime = 0
	}
	gp.waitsince = 0
	gp.preempt = false
...
top:
	...
	// Zero the queueing delay before blocking in findRunnable so that
	// idle Ps don't hold a stale value.
	pp.maxQueueDelay.Store(0)
